...
for e = 1:nelem
    ...
    %Very similar to the conductivity matrix at A2

    for i = 1:nnodeE 

        a = Nod2DOF(i,ndim); %Vector of DOF of node i
        A = Nod2DOF(CN(e,i),ndim); %Ask the connectivity matrix the dof for e at i

        for j = 1:nnodeE

            b = Nod2DOF(j,ndim);
            B = Nod2DOF(CN(e,j),ndim);
            K(A,B) = K(A,B)+Ke(a,b);
        end
